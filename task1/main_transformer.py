...
from libs.task1_model import TransformerModel
...
def train(is_with_tensorboard=True, seed=42):
    ...
    model = TransformerModel(
        transformer, input_len=INPUT_DIM, target_len=OUTPUT_DIM, n_features=1, is_with_pe=False)
    ...
    X_training.unsqueeze_(-1)
    X_test.unsqueeze_(-1)
    y_training.unsqueeze_(-1)
    y_test.unsqueeze_(-1)
    
    training_loader, test_loader = create_dataloader(torch.cat([X_training, y_training], dim=1), X_test, y_training, y_test, batch_size=BATCH_SIZE)
